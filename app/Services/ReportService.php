...
class ReportService
{
    ...
    public function getTitipanSummary($startDate, $endDate, $sellerId, $filters = [])
    {
        ...
        $base = fn() => TransactionItem::whereHas('product', function($q) use ($sellerId, $search) {
                $q->where('type', 'siswa')
                  ->when($sellerId, function($q2) use ($sellerId) {
                      return $q2->where('seller_id', $sellerId);
                  })
        ...
        $dateBetween = fn($q, $from, $to) => $q->whereHas('transaction', fn($t) =>
            $t->whereBetween(DB::raw('DATE(transaction_date)'), [$from, $to]));
