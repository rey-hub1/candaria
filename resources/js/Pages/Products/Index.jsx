...
const inputCls = "w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition";
    ...
    <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
            ...
            {data.type === 'kantin' ? (
                ...
                <FormField label="Harga Jual">
                    <div className="flex items-center h-[38px] px-3 bg-primary-50 border border-primary-100 rounded-lg text-xs text-primary-700 font-semibold">
        ...
        <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
            ...
            <button type="submit" disabled={processing}
                className="px-5 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold text-sm rounded-lg shadow-sm transition disabled:opacity-50 disabled:cursor-not-allowed">
...
export default function Index({ products = { data: [], links: [], total: 0 }, filters = {}, categories = [], sellers = [] }) {
    ...
    const { data: editData, setData: setEditData, post: postEdit, processing: editProcessing, errors: editErrors, reset: editReset } = useForm({
        ...
        cost_price: '', selling_price: '', seller_id: '', stock: '', image: null, current_image_url: null,
    });
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white px-5 py-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    ...
                    <button onClick={() => setAddModal(true)}
                        className="shrink-0 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold text-sm rounded-lg shadow-sm transition inline-flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                        </svg>
                        Tambah Produk
                ...
                {products.data.length === 0 ? (
                    <div className="bg-white rounded-xl border border-dashed border-slate-300 p-16 text-center">
                        ...
                        <button onClick={() => setAddModal(true)} className="mt-4 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold rounded-lg transition">
                                ...
                                <div key={p.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-slate-100">
                                        ...
                                        <div className="text-center">
                                            ...
                                            <p className={`text-xs font-extrabold ${p.stock <= 5 ? 'text-rose-600' : p.stock <= 15 ? 'text-amber-600' : 'text-primary-600'}`}>{p.stock}</p>
                                        </div>
                                    </div>

                                    <div className="flex gap-2 mt-3">
                                        <button onClick={() => openEditModal(p)}
                                            className="flex-1 py-1.5 bg-slate-100 hover:bg-primary-50 hover:text-primary-700 text-slate-700 font-bold text-xs rounded-lg transition flex items-center justify-center gap-1.5">
                                        ...
                                        <button onClick={() => handleDelete(p.id)}
                                            className="flex-1 py-1.5 bg-slate-100 hover:bg-rose-50 hover:text-rose-700 text-slate-700 font-bold text-xs rounded-lg transition flex items-center justify-center gap-1.5">
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                            ...
                                            <tr key={p.id} className="hover:bg-slate-50/70 transition group">
                                                ...
                                                <td className="px-5 py-3.5 whitespace-nowrap text-center">
                                                    <div className="inline-flex gap-1.5 opacity-70 group-hover:opacity-100 transition">
                                                        <button onClick={() => openEditModal(p)}
                                                            className="inline-flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:bg-primary-50 hover:border-primary-300 hover:text-primary-700 text-slate-600 font-semibold text-xs rounded-lg transition shadow-sm">
