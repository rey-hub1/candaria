...
- `Create.jsx` mobile polish: floating rounded cart bar (`bottom-[4.5rem] inset-x-3 rounded-2xl`), cart drawer is 92dvh (not full screen) with a "Kosongkan" button in its header (closes drawer after clearing), search Reset button hidden on mobile (inline × inside input instead), product grid has `pb-24` on mobile so the floating bar doesn't cover last row.
